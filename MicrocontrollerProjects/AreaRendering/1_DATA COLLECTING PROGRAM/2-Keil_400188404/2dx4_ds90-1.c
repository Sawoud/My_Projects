...
void storeInFile(float x, float y, float z);
...
/* Those basic I2C read functions can be used to check your own I2C functions */
  ...
  status = VL53L1X_StartRanging(dev);   /* This function has to be called to enable the ranging */
	Status_Check("StartRanging", status);
			...
			int flag = 0; // this flag variable is used later on in the program
			
	while(1){
					GPIO_PORTL_DATA_R = 0b00000000; // when no data is being collected, LED is off
		while((GPIO_PORTM_DATA_R&0b00100000) == 0){// once button is pressed, program starts rotating stepper motor and collecting data
			GPIO_PORTL_DATA_R = 0b00000010;// LED gets turned on
			flag++;// flag  gets incremented
		if(flag%2 != 0){ // if the flag is not divisable by 2 then  the motor starts rotating clockwise
				for(int i = 0; i<512; i++){
	...
	SysTick_Wait10ms(1);
				...
				if (i%64 == 0)//when the stepper motor rotates 45 degrees it goes into the sensor's data collecting routine
{	
	SysTick_Wait10ms(2);
//ToF_UART function 
  	for(int j = 0; j < 1; j++) {
//  while(1){ /* read and display data */
    ...
    UART_printf(printf_buffer); // mesurment gets sent
    UART_printf("x");// and a 'x' also sent after the number
	  ...
	  SysTick_Wait10ms(50);
				...
				if(flag%2 == 0){// if the flag is divisavle by 2 then the motor starts rotating counter-clockwise
						for(int i = 0; i<512; i++){
...
//  while(1){ /* read and display data */
	  ...
	  status = VL53L1X_GetSpadNb(dev, &SpadNum);
	  status = VL53L1X_ClearInterrupt(dev); /* clear interrupt has to be called to enable next interrupt*/
    sprintf(printf_buffer,"%u",Distance); // mesurment gets sent
    UART_printf(printf_buffer);
    UART_printf("x");// and an 'x' also gets sent after the number
...
void PortL_Init(void){
	...
	GPIO_PORTL_DIR_R |= 0b00000010;    // output    								
...
void PortM_Init(void){
	...
	GPIO_PORTM_DIR_R |= 0b00001111;    // output    								
  GPIO_PORTM_DEN_R |= 0xFF; 		
	return;
}

void PortJ_Init(void){
    SYSCTL_RCGCGPIO_R |= SYSCTL_RCGCGPIO_R8;                // activate clock for Port N
    while((SYSCTL_PRGPIO_R&SYSCTL_PRGPIO_R8) == 0){};    // allow time for clock to stabilize
    GPIO_PORTJ_DIR_R &=  0b00000000;                                        // make PG0 in (HiZ)
  GPIO_PORTJ_AFSEL_R &= ~0b00000011;                                     // disable alt funct on PG0
  GPIO_PORTJ_DEN_R |=    0b00000011;                                        // enable digital I/O on PG0
  GPIO_PORTJ_AMSEL_R &= ~0b00000011;                                     
